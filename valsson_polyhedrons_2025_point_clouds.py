...
'''
    Module written to present point cloud from 2025 Valsson study.

    This implementation uses MAtplotlib, found to be lacking as it does not correctly render
    the object order in 3D.
...
def present_model():
    out_dir = 'PLY_point_cloud_restored'
    p_clouds = [ prep_p_cloud( f ) for f in files_in_folder( out_dir ) ]
    ...
    lgnd = plt.legend( by_label.values(), by_label.keys(), title="Point group center density", alignment='left', ncol=3, markerscale=3, bbox_to_anchor=(.5, .8), fontsize=15, title_fontsize=18 )

    #plt.savefig('valsson_2025_cloud.png', dpi=100)
    plt.show()
